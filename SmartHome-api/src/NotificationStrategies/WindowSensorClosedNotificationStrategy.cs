...
public sealed class WindowSensorClosedNotificationStrategy(
    ...
    INotificationCreator notificationService) : INotificationStrategy
{
    ...
    private const string WindowClosedEvent = "window-closed";

    public bool CanHandle(DeviceType deviceType, string triggeringEvent)
    {
        return deviceType == DeviceType.Sensor && triggeringEvent == WindowClosedEvent;
    }
    ...
    public void GenerateNotifications(Guid hardwareId, string? additionalData = null)
    {
        ...
        foreach (var member in homeMembers)
        {
            ...
            {
                var args = new CreateNotificationArgs(homeId, member.Id, hardwareId, "Window Closed");
