...
public class PersonDetectionNotificationStrategy(
    ...
    INotificationCreator notificationService) : INotificationStrategy
{
    ...
    private const string PersonDetectedEvent = "person-detected";

    public bool CanHandle(DeviceType deviceType, string triggeringEvent)
    {
        return deviceType == DeviceType.Camera && triggeringEvent == PersonDetectedEvent;
    }
    ...
    public void GenerateNotifications(Guid hardwareId, string? additionalData = null)
    {
        ...
        var user = userService.GetUserByEmail(additionalData ?? string.Empty);
        ...
        foreach (var member in homeMembers)
        {
            ...
            {
                var args = new CreateNotificationArgs(homeId, member.Id, hardwareId, "Person Detected: " + user.Name + " " + user.Surname + ": " + additionalData);
